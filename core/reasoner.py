...
规则：
- 计划是行动参考，不是固定流程；你必须根据最新观察重新判断。
- 不要因为关键词出现就机械调用工具。
- 工具返回结果后必须重新分析。
- 信息不足时可以继续行动；足够时 ANSWER。
- 外部事实、最新信息、论文结论尽量获得证据；数学推导不必强行搜索。
- 发现前提、概念、逻辑或范围问题时，应优先处理问题本身。
- 不确定时不要编造。
...
"""
    ...
    def decide(self, state):
        with debug.scope("AgentReasoner", f"DECIDE → step={state.step_count + 1}"):
            ...
            debug.log("AgentReasoner", f"PROMPT → {len(prompt)} chars")
            excluded: set[str] = set()
            candidates = self.model_router.select_candidates(capability="reasoning", allow_paid=self.allow_paid, exclude=excluded)
            if not candidates:
            ...
            for model in candidates:
                ...
                try:
                    ...
                    debug.log("AgentReasoner", f"CALL LLM → {model.name}")
                    raw = client.generate(self.SYSTEM_PROMPT, prompt, json_mode=True)
                    debug.log("AgentReasoner", f"LLM RETURN → {len(raw)} chars")
                    ...
                    if decision.tool:
                        debug.log("AgentReasoner", f"TOOL → {decision.tool}")
                    return decision
                except Exception as exc:
    ...
    def _build_prompt(self, state):
        ...
        payload = {
            ...
            "previous_steps": observations, "evidence": state.evidence, "claims": state.claims,

...
class StudyAgent:
    ...
    def run(self, question: str, student_state=None) -> AgentState:
        with debug.scope("StudyAgent", "RUN"):
            ...
            try:
                state.task_analysis = self.task_analyzer.analyze(
                    question,
                    state.student,
                )
                ...
                debug.log(
                    "StudyAgent",
                    f"PLAN → {' → '.join(state.plan.actions)}",
                )
            except Exception as exc:
                state.error = (
                    f"Task Analysis 执行失败：{type(exc).__name__}: {exc}"
                )
            ...
            debug.log(
                "StudyAgent",
                f"LOOP FINISHED → steps={state.step_count}",
            )
